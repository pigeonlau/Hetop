...
public class TypeCaster {
    ...
    public static Object cast(Object value, Type destType) {
        return doCast(value, destType);
    }


    public static <T> T cast(Object value, Class<T> destClass) {
        @SuppressWarnings("unchecked")
        T rst = (T) doCast(value, destClass);
        return rst;
    }

    //判断两个类型能否转换

    public static boolean canCast(Class<?> fromClass, Type destType) {
        if (fromClass == null) {
    ...
    public static boolean canCast(Object value, Type destType) {
        if (destType == null) {
            throw new MTypeCastException(
                    String.format("转换目标类型不能为null"));
        }
    ...
    private static Object castNullToDestType(Type destType) {
    ...
    //类型转换
    private static Object doCast(Object value, Type destType) {
        ...
        if (value == null) { //要转换的对象为null，调用处理null转换的方法
            return castNullToDestType(destType);
        }
