...
public class SetLoader {
	 ...
	 * @return
	 */
	public static Object readSetData(
			MotpLoader loader,
			MByteBuffer buffer,
			Type destType) {
		...
		if (destType instanceof ParameterizedType) {
			...
			do {
				...
				if (pt.getRawType() == Set.class 
					...
					return SetLoader.readSetDataAsHashSet(loader, buffer, size, typeArgs[0]);
				}
				
				if (pt.getRawType() == List.class 
						|| pt.getRawType() == ArrayList.class) {
					return SetLoader.readSetDataAsArrayList(loader, buffer, size, typeArgs[0]);
				}
			} while (false);
			...
			if (clazz.isArray()) {
				...
				return SetLoader.readSetDataAsArray(
						loader, buffer, size, clazz.getComponentType());
			} else {
				if (clazz == Set.class 
						|| clazz == HashSet.class) {
					return SetLoader.readSetDataAsHashSet(
							loader, buffer, size);
				}
				
				if (clazz == List.class 
						|| clazz == ArrayList.class) {
					return SetLoader.readSetDataAsArrayList(
							loader, buffer, size);
				}
				/*
				 * 目前不支持此种情况
			...
			return SetLoader.readSetDataAsArray(loader, buffer, size, componentClass);
	...
	private static HashSet<Object> readSetDataAsHashSet(
			MotpLoader loader, 
			MByteBuffer buffer,
			int size,
			Type eleType) {
	...
	private static ArrayList<Object> readSetDataAsArrayList(
			MotpLoader loader,
			MByteBuffer buffer, 
			int size,
			Type eleType) {
	...
	private static Object readSetDataAsArray(
			MotpLoader loader, 
			MByteBuffer buffer, 
			int size,
			Class<?> componentType) {
	...
	public static void skipSetData(
			MotpLoader loader, 
			MByteBuffer buffer, 
			int size) {
		for (int i = 0; i < size; i++) {
