...
public class MByteBuffer { //32位系统，页表一页大小为4kB
	...
	public MByteBuffer(byte[] buffer) { //有参构造函数（根据传入的字节数组构造）
		super();
		int len = buffer.length;
		len = len / PAGE_SIZE * PAGE_SIZE + PAGE_SIZE; //先除再乘可以去除余数，使长度是4k的倍数
		this.buffer = new byte[len]; //设置自身buffer大小为传入buffer大小加4kB（2kB的倍数）
		this.size = buffer.length; //设置size为传入buffer数组长度大小
		System.arraycopy(buffer, 0, this.buffer, 0, buffer.length); //把传入buffer的内容复制到自身buffer中
	}
